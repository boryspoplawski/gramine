...
int edmm_restrict_pages_perms(uint64_t addr, size_t count, uint64_t prot) {
    ...
    while (addr < end) {
        struct sgx_enclave_restrict_permissions params = {
            .offset = addr - g_pal_enclave.baseaddr,
            .length = size,
            .permissions = prot,
        };
...
int edmm_modify_pages_type(uint64_t addr, size_t count, uint64_t type,
    ...
    if (bitmap_missing_pages) {
        memset(bitmap_missing_pages, 0, UDIV_ROUND_UP(count, 64) * sizeof(*bitmap_missing_pages));
    }
    ...
    while (i < count) {
        ...
        if (ret < 0) {
            ...
            if (ret == -EFAULT) {
                if (bitmap_missing_pages) {
                    bitmap_missing_pages[i / 64] |= 1ul << i % 64;
                }
                i += 1;
                continue;
            }
...
int edmm_remove_pages(uint64_t addr, size_t count, uint64_t* bitmap_missing_pages) {
    ...
    while (i < count) {
        ...
        if (ret < 0) {
            ...
            if (ret == -EFAULT && params.count) {
                /* Next page is probably missing. */
                continue;
            }
            return ret;
        }
