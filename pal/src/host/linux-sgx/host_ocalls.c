...
static long sgx_ocall_edmm_modify_pages_type(void* _args) {
    ...
    return edmm_modify_pages_type(args->addr, args->count, args->type, args->bitmap_missing_pages);
}

static long sgx_ocall_edmm_remove_pages(void* _args) {
    struct ocall_edmm_remove_pages* args = _args;
    return edmm_remove_pages(args->addr, args->count, args->bitmap_missing_pages);
}
